 ...
 if (buttonsStatus.length > 0) {
  ...
  document.addEventListener('click', async function (event) {
    ...
    if (updateButton) {
      ...
      try {
        const response = await fetch(`/staff/order/${orderId}/status`, {
          method: 'POST',
    ...
    if (detailButton) {
      ...
      if (orderId) {  
            window.location.href = `/staff/order/${orderId}`;
      }
    }
  });
  ...
  document.getElementById('applyBulkAction').addEventListener('click', async function() {
    ...
    try {
        const response = await fetch('/staff/order/apply-bulk-action', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            window.location.replace('/staff/order');         
        } else {
